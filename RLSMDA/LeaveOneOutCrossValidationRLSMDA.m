function [FinalCandiRankList,AUC] = LeaveOneOutCrossValidationRLSMDA(DataPath,mi2diNetwork)
...
for i=1:length(Disease_name) %#ok<USENS>
    ...
    for j=1:length(miRNABin)
        ...
        Tempmi2diNetwork=mi2diNetwork;
        CandiMiRNA=setdiff(miRNA_name,miRNABin);
        TempCandi=RLSMDA([miRNABin{j};CandiMiRNA],miRNA_name,MiFunSim,DiPheSim,Tempmi2diNetwork,i);
